...
public class SerialInOutManager extends SerialInputOutputManager implements SerialInputOutputManager.SerialInListener{
    ...
    public void broadcastData(String data) {
        ...
        if(data.equals("j n")){
            broadcast.requestNewTask();
        } else {
            if (data.contains("t") || data.contains("m") || data.contains("g")) {
                //broadcast.broadcastCalculateErrorCommand();
                broadcast.broadcastWheelEncoderInformation(data);
            }
        }
        parseData(data);
        oldData = "";
    ...
    public void setImageLearned(boolean imageLearned) {
        this.imageLearned = imageLearned;
    }

    double oldOrientation = 0;
    ...
    private void parseData(String data){
        ...
        /*
        if(orientation - oldOrientation >= 5 && imageLearned){
    ...
    private void parseNewIncomingData(String newData) {
        ...
        if (newData.contains("nt") || newData.contains("nm") || newData.contains("nx") || newData.contains("nj") || newData.contains("ng")) {
        ...
        //was split so we pass it on to the attemptSplitRepair method.
            ...
            if (newData.contains("t") || newData.contains("m") || newData.contains("x") || newData.contains("j") || newData.contains("g")) {
    ...
    private void attemptFuseRepair(String newData) {
        ...
        if (firstHalf.contains("t") || firstHalf.contains("m") || firstHalf.contains("x") || firstHalf.contains("j") || firstHalf.contains("g")) {
            // Complete
    ...
    private void attemptSplitRepair(String newData) {
        ...
        if (oldData.contains("n")) {
            if (oldData.contains("t") || oldData.contains("m") || oldData.contains("x") || oldData.contains("j") || oldData.contains("g")) {
    ...
    public void parseOutMessage(String message)
    {
        ...
        try
        {
            ...
            switch(tokens[0])
            {
                ...
                case "go":
                    go(Integer.valueOf(tokens[1]), Integer.valueOf(tokens[2]));
                    break;
    ...
    /**
     * Order AntBot to move at a certain speed (cm/sec)
     */
    public void go(double speed1, double speed2)
    {
        String str = "g " + speed1 + " " + speed2 + " n";
        sendData(str);

        Log.i(TAG, "AntBotController GO '" + str + "'");
