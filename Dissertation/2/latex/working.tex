...
\raggedright
\textbf{DRAFT NOTE:} Footnotes marked with [DRAFT] are there purely to comment
on something that may be relevant, for example, where a paragraph or idea
will make more sense once the full dissertation is written, or simply to note
points where I am confused and could do with specific feedback/clarification.
These will be removed from the final version. Footnotes not marked with [DRAFT]
are actual footnotes, and will be kept.
\newline
\par
...
centre, performing all higher sensory processing and decision making. Once the
algorithm running on the phone has decided on a course of action, it sends a
command via a serial connection to the Arduino board; the board, then parses
the command and translates it into a sequence of motor commands which are sent
to the built-in motor board in the chassis.
