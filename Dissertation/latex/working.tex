...
\subsubsection{Filtering}
Here a dense optical flow field is used, and in order to explain the computation, it is important
...
coordinate, and $M_{y,x}[1]$ gives the displacement in the $y$ axis.
\newline
...
our method for performing the computation and avoidance was modified slightly. Instead of creating
...
\cite{Scimeca2017} to $\pm 16^{circ}$. It was found that the size of this angle affected the region of the %Need to fact check this!!!
...
want the robot to avoid obstacles to the front, obstacles to the sides should not trigger responses. The filter
...
retrieved from \textit{calcOpticalFlowFarneback()} to the pixel value and compare it to the filter. 
