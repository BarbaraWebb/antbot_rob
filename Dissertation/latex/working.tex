...
paper, we will focus on using (and discuss extensions for) an existing model for visual navigation
in ants using
...
outputs $N_a$ to a single line which represents our EN and our reward signal. The output corresponds to the number of
KCs that are activated by the input pattern, however, learning only occurs on receiving the reward signal (a slight
modification of Willshaw's original network).
...
$d$ and $v$ are as given in Equation (\ref{eqn:lowttc}).
...
In three of five runs in Arena 8, the robot reacted correctly initially, but in moving away from the wall it would lose sight of it and ultimately
turn back towards it resulting in a collision; one run was a complete failure. In all cases in Arena 9, the robot reacted to one side of the
