...
paper, we will focus on using (and discussing extensions for) an existing model for visual navigation
in ants using
...
establish a baseline against which future iterations can work while modifying the model. We will use
...
(the best model used by \textit{Ardin et al.}) which may suggest that the model is
...
balance strategy is similar in idea to the filtering system described in Section \ref{sec:methods}, though
...
their method of computing the FOE and did not translate well into AntBot giving a wildly fluctuating FOE.
A more general method for computing the FOE is given by \cite{ODonovan2005}:
...
and a tuning factor is applied simply to make the numbers returned easier to handle.
\newline
...
speeds, and while this is plausible from a biological standpoint \cite{Wittlinger1965}, we are currently
...
\begin{figure}
  ...
  \caption{
    ...
    section of the robot's view, the tussock is comparitively small in the visual frame. This image was taken without the arena.
  }
\end{figure}
...
which experienced serious error, though it recovered. The homeward experiments experienced only 2 errors over the five memory runs.
\newpage
...
everything in sync. For basic operation, this presents no problems, but a more refined control system (
from the Android component) is made difficult and accurate timing in threads becomes impossible. Reactionary
behaviours will work initially, but ultimately the delays cause a delayed reaction (one of the main reasons the
...
was noted in early scanning tests, and the later homeward route. The turning code is based purely based on
...
or compass could remove this problem entirely. In this project, we discovered just how seriously simple
